...
/*
 * tannenba -
 * Check to see if user is allowed to see this page.
 * Print error message if non-trusted user is trying to view a 
 * private wiki page.
 * return 1 if ok for user to view this page, else 0
 */
static int
okRdThisWikiPage(const char *page)
{
...
void wikitool(void){
  ...
  /* tannenba - only trusted users can view private pages */
  if ( !okRdThisWikiPage(zPage) ) return;
