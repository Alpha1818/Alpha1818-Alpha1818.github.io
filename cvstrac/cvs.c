...
static void check_path(int nErr, const char *zProg){
  ...
  for(z=zPath; z && *z; z=zEnd){
    ...
    bprintf(zBuf, size, OS_VAL("%s/%s", "%s\\%s.exe"), z, zProg);
...
static int cvs_history_update(int isReread){
  ...
  unsigned long t;
  const char *zModule = db_config("module", 0);
  regex_t rModule;
  ...
  */
  ...
  if( zModule ){
    /*
    ** In order to get backwards compatibility, we ensure that the regex
    ** is anchored on the left. Unless someone's trying something silly
    ** like creating a project that only has certain file extensions,
    ** this shouldn't be a problem.
    */
    char *zC = mprintf("%s%s", (zModule[0]=='^') ? "" : "^", zModule);
    if( regcomp(&rModule, zC, REG_EXTENDED|REG_NOSUB) ){
      error_init(&nErr);
      @ <li><p>Unable to compile module regex %h(zC)</p></li>
      db_execute("COMMIT");
      return -1;
    }
  }
  ...
  while( fgets(zLine,sizeof(zLine),in) ){
    ...
    */
    if( (c=='A' || c=='M' || c=='R') && nField>=5 ){
      char *zFn = mprintf("%s/%s", azField[2], azField[4]);
      if( zModule && regexec(&rModule,zFn,0,0,0) ){
        /*
        ** Filter out any files which don't match the regular expression
        ** defining the module.
        */
        continue;
      }
      db_execute("INSERT INTO fc VALUES(%d,'%q','%q',%d,'%q')",
                 tm, azField[0], zFn,
                 ((c=='A') ? 1 : ((c=='R') ? 2 : 0)),
                 azField[3]);
    }else if( zLine[0]=='T' ){
      ...
      */
      ...
      if( zModule && regexec(&rModule,azField[4],0,0,0) ){
        /*
        ** Tag directory not in the modules regex. Ignore.
        */
        continue;
      }
      ...
      if( azField[2][0]=='A' || azField[2][0]=='D' ){
        ...
        */
        ...
        /* For whatever reason, we may not know about _all_ tags in the
        ** repository. So if we can't match it, just go with the date
        ** we've got.
        */
        if( z ) date = atoi(z);
        if( date==0 ) continue;
  ...
  */
  ...
  minTime -= 3600;
  /* CVSNT is reading time in local instead of GMT which is bug ! */
  pTm = OS_VAL(gmtime(&minTime), localtime(&minTime));
  ...
  maxTime += 3600;
  /* CVSNT is reading time in local instead of GMT which is bug ! */
  pTm = OS_VAL(gmtime(&maxTime), localtime(&maxTime));
  ...
  for(i=0; azFileList[i]; i++){
    ...
    */
    ...
    azSubst[6] = 0;
    zCmd = subst(db_config("rlog", OS_VAL("rlog '-d%TR' '%F' 2>/dev/null",
                                          "rlog \"-d%TR\" \"%F\" 2>NUL")), azSubst);
  ...
  */
  ...
  if( oldVersion[0]==0 ){
    zTemplate = db_config("filelist",
      OS_VAL("co -q -p'%V' '%F' | diff -c /dev/null - 2>/dev/null",
             "cvs -q rcsfile co -p\"%V\" \"%F\" 2>NUL | diff -c NUL - 2>NUL"));
    azSubst[2] = "V";
    ...
    zTemplate = db_config("filediff", OS_VAL("rcsdiff -q '-r%V1' '-r%V2' -u '%F' 2>/dev/null",
                                             "cvs -q rcsfile rcsdiff \"-r%V1\" \"-r%V2\" -u \"%F\" 2>NUL"));
    azSubst[2] = "V1";
...
static void raw_diff_versions(
  ...
  */
  ...
  if( is_dead_revision(zRelFile,oldVersion) ){
    zTemplate = OS_VAL("co -q -kk -p'%V' '%F' | diff -u /dev/null - -L'%R' 2>/dev/null",
                       "cvs -q rcsfile co -kk -p\"%V\" \"%F\" 2>NUL | diff -u NUL - -L\"%R\" 2>NUL");
    azSubst[4] = "V";
    azSubst[5] = newVersion;
    azSubst[6] = 0;
  }else if( is_dead_revision(zRelFile,newVersion) ){
    zTemplate = OS_VAL("co -q -kk -p'%V' '%F' | diff -u - /dev/null -L'%R' 2>/dev/null",
                       "cvs -q rcsfile co -kk -p\"%V\" \"%F\" 2>NUL | diff -u - /dev/null -L\"%R\" 2>NUL");
    azSubst[4] = "V";
    azSubst[5] = oldVersion;
    azSubst[6] = 0;
  }else{
    zTemplate = OS_VAL("cd %r && rcsdiff -q -kk '-r%V1' '-r%V2' -u '%R' 2>/dev/null",
                       "cvs -q rcsfile rcsdiff -kk \"-r%V1\" \"-r%V2\" -u \"%F\" 2>NUL");
    azSubst[4] = "V1";
    azSubst[5] = oldVersion;
    azSubst[6] = "V2";
