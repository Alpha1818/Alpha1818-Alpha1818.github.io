...
#ifndef SQLITE_FUNCTION
  if( SQLITE_VERSION_NUMBER <= 3003007
    && sqlite3_libversion_number() >= 3003008
  ){
    /* 3.3.8 introduced [3365], which added the SQLITE_FUNCTION symbol to
    ** the authorizer. If CVSTrac was built against an older version
    ** missing that symbol, things can get a little messed up since routine
    ** queries expect basic functions will work.
    */
    cgi_reset_content();
    cgi_set_status(200, "OK");
    cgi_set_content_type("text/html");
    @ <h1>SQLite3 Error</h1>
    @ SQLite3 Library Version has changed!
    @
    @ You must recompile CVSTrac.
    cgi_append_header("Pragma: no-cache\r\n");
    cgi_reply();
    exit(1);
  }
...
@                             -- 4: trusted program markup,
@                             -- 5: trusted program block
...
/*
** The following are additions to the schema for version 2.3. A "reason"
** field as added to wiki, a wiki comment field to inspect, and a
** description field to reportfmt.
*/
static char zSchemaChange_2_3[] =
@ BEGIN;
@ CREATE TEMP TABLE old_wiki AS SELECT * FROM wiki;
@ DROP TABLE wiki;
@ CREATE TABLE wiki(
@   name text,            -- Name of this page
@   invtime int,          -- Inverse timestamp: seconds *before* 1970
...
@   ipaddr text,          -- IP Address of "who"
@   text clob,            -- Text of the page
@   reason text,          -- Reason for the last change
@   UNIQUE(name,invtime)
@ );
@ INSERT INTO wiki(name,invtime,locked,who,ipaddr,text)
@       SELECT name,invtime,locked,who,ipaddr,text FROM old_wiki;
@ CREATE TEMP TABLE old_inspect AS SELECT * FROM inspect;
@ DROP TABLE inspect;
@ CREATE TABLE inspect(
@   cn integer not null,      -- The check-in that was inspected
@   inspecttime int not null, -- Time that the inspection occurred
...
@   ticket int,               -- Ticket related to this inspection - or NULL
@   result text,              -- "passed", "failed", or other comments, or NULL
@   comment text              -- wiki formatted discussion
@ );
@ INSERT INTO inspect(cn,inspecttime,inspector,ticket,result)
@       SELECT cn,inspecttime,inspector,ticket,result FROM old_inspect;
@ CREATE TEMP TABLE old_reportfmt AS SELECT * FROM reportfmt;
@ DROP TABLE reportfmt;
@ CREATE TABLE reportfmt(
@    rn integer primary key,  -- Report number
@    owner text,              -- Owner of this report format (not used)
@    title text,              -- Title of this report
@    description text,        -- Wiki description/explanation of this report
@    cols text,               -- A color-key specification
@    sqlcode text             -- An SQL SELECT statement for this report
@ );
@ INSERT INTO reportfmt(rn,owner,title,cols,sqlcode)
@       SELECT rn,owner,title,cols,sqlcode FROM old_reportfmt;
@ UPDATE config SET value='2.3' WHERE name='schema';
@ COMMIT;
;
        ...
        */
        if( !db_exists("SELECT * FROM xref WHERE tn=%d AND cn=%d", tn, cn) ){
          ...
          bprintf(zSql, sizeof(zSql), "INSERT INTO xref(tn,cn) VALUES(%d,%d)",
                  tn, cn);
          //if(!db_exists( "SELECT * FROM xref WHERE tn=%d",tn)){
            char **az_oldval_chng = db_query("SELECT * FROM ticket WHERE tn=%d",tn);
            if(strcmp(az_oldval_chng[2],"new")==0){
              db_execute("UPDATE ticket SET status='active' WHERE tn=%d",tn);
              char **az_new_active_chng = db_query("SELECT * FROM ticket WHERE tn=%d", tn );
              db_execute("INSERT INTO tktchng(tn,user,chngtime,fieldid,oldval,newval) "
              "VALUES(%d,'%q',%d,'%s','%q','%q')", tn, g.zUser, now, "status", az_oldval_chng[0], az_new_active_chng[0]);
            }
         // }
          db_execute(zSql);
        }
...
/*
** Update the database schema from version 2.2 to version 2.3
*/
void db_upgrade_schema_23(void){
  db_execute(zSchemaChange_2_3);
}
