    ...
    switch( atoi(az[i+1]) ){
      case 1: { /* A check-in or milestone */
        ...
        if( az[i+3][0] && az[i+3][0]!='0' ){
          ...
          if( rssDetail>=9 ) {
            /*
            ** Construct a zMsg containing the list of changed files. In the
            ** interest of bandwidth and simplicitly we're _not_ going to try
            ** to encode as much info as a full /chngview... Just enough to
            ** give the reader some idea of what files got touched.
            */

            int j, nLen = 0, nMaxLen = 0;
            char **azFile = db_query("SELECT filename FROM filechng "
                                     "WHERE cn=%q ORDER BY +filename",
                                     az[i+5]);
            for(j=0; azFile[j]; j++){
              nMaxLen += strlen(azFile[j]);
            }
            /* make room for each filename, plus a ", " between each, plus
            ** a bit of padding, plus the HTML wrapping it all.
            */
            nMaxLen += j * 4 + 100;
      ...
      case 8: { /* An inspection */
        /* result is not wiki markup, but might have HTML characters */
        zMsg = mprintf("%h", az[i+6]);

        if( rssDetail>=9 ){
          /* not the optimal way to find an inspection, I know */
