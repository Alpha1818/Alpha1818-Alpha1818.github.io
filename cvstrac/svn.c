...
static int svn_did_repository_change( const char * zRoot, time_t since ){
  struct stat statbuf;
  char *zFilename = mprintf(OS_VAL("%s/db/revisions",
                                   "%s\\db\\revisions"), zRoot); /* bdb */
  if( stat(zFilename, &statbuf) ){
    free(zFilename);
    zFilename = mprintf(OS_VAL("%s/db/current",
                               "%s\\db\\current"),zRoot); /* fsfs */
    if( stat(zFilename, &statbuf) ){
      free(zFilename);
...
static int svn_history_update(int isReread){
  ...
  */
  zCmd = mprintf(OS_VAL("svnlook youngest '%s' 2>&1",
                        "svnlook youngest \"%s\" 2>NUL"), quotable_string(zRoot));
  ...
  */
  for(nRev=nBaseRevision+1; nRev<=nHeadRevision; nRev++){
    ...
    */
    zCmd = mprintf(OS_VAL("svnlook info -r %d '%s' 2>/dev/null",
                          "svnlook info -r %d \"%s\" 2>NUL"),
    ...
    */
    zCmd = mprintf(OS_VAL("svnlook changed -r %d '%s' 2>/dev/null",
                          "svnlook changed -r %d \"%s\" 2>NUL"),
                   nRev, quotable_string(zRoot));
    in = popen(zCmd, "r");
    if( in==0 ){
      error_init(&nErr);
        ...
        if( nRev>1 ){
          ...
          */
          ...
          if( zFilename[nPos]=='/' ){
            ...
            char *zCurrPath = NULL;
            int i=0, nHistErr=0;
            zCmd = mprintf(OS_VAL("svnlook history -r %d '%s' '%s' 2>/dev/null",
                                  "svnlook history -r %d \"%s\" \"%s\" 2>NUL"), nRev, 
...
static int svn_diff_versions(
  ...
  */
  ...
  azSubst[10] = 0;
  if( file_cat==1 ){
    zTemplate = db_config("filelist", OS_VAL("svnlook cat -r '%V' '%RP' '%F' 2>/dev/null",
                                             "svnlook cat -r \"%V\" \"%RP\" \"%F\" 2>NUL"));
  }else{
    zTemplate = db_config("filediff", OS_VAL("svnlook diff -r '%V2' '%RP' 2>/dev/null",
                                             "svnlook diff -r \"%V2\" \"%RP\" 2>NUL"));
  }
...
static int svn_dump_version(const char *zVersion, const char *zFile,int bRaw){
  ...
  if( !zVersion || !zVersion[0] ){
    zCmd = mprintf(OS_VAL("svnlook cat '%s' '%s' 2>/dev/null",
                          "svnlook cat \"%s\" \"%s\" 2>NUL"),
      db_config("cvsroot", ""), quotable_string(zFile) );
  }else{
    zCmd = mprintf(OS_VAL("svnlook cat -r '%s' '%s' '%s' 2>/dev/null",
                          "svnlook cat -r \"%s\" \"%s\" \"%s\" 2>NUL"),
...
static int svn_diff_chng(int cn, int bRaw){
  ...
  zRoot = db_config("cvsroot", "");
  zCmd = mprintf(OS_VAL("svnlook diff -r '%s' '%s' 2>/dev/null",
                        "svnlook diff -r \"%s\" \"%s\" 2>NUL"),
...
/*
** Read svnserve users file and record information gleaned from that file
