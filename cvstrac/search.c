...
void search_page(void){
  ...
  if( zPattern && zPattern[0] && strlen(zPattern)<100 ){
	...
	if( zPattern[0]=='#' ){
		i = atoi(&(zPattern[1]));
		az = db_query("SELECT type FROM ticket WHERE tn=%d", i);
		if( az[0]!=0 ) cgi_redirect(mprintf("tktview?tn=%d", i));
	}
  ...
  @ </p>Note: To jump directly to specific ticket, enter a # symbol followed by ticket number, e.g. #3255
  @ </form>
    ...
    for(i=n=0; azResult[i]; i+=5, n++){
      ...
      switch( atoi(azResult[i]) ){
        case 1: /* ticket */
		  ...
		  {
			/* Do not show_text if ticket is private and user is non-admin */
			char **az;
			if ( !g.okAdmin ) {
				az = db_query("SELECT extra3 FROM ticket WHERE tn=%d", atoi(azResult[i+2]));
				if ( az && az[0] && strcasecmp(az[0],"private")==0 ) {
					show_text = 0;
				} 
			}
			if ( show_text ) {
        ...
        case 3: /* wiki */
		  ...
		  {
			  /* Do not show_text is wiki page is private and user is non-admin */
			  char *page = azResult[i+3];
			  if ( page && page[0] &&
				 ( strncasecmp(page,"private",strlen("private"))==0 || 
				   strncasecmp(page,"confidential",strlen("confidential"))==0 || 
				   strncasecmp(page,"hiring",strlen("hiring"))==0 ) &&
				   !g.okAdmin )
			  {
				  show_text = 0;
			  }
      ...
      @ <br>%s("This attachment is private.")</p>
	  }
    }
    if( nPage || n>=30 ){
      @ <hr><p align="center">
