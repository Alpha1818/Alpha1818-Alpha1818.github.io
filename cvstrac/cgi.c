...
** incorrect.
**
** Windows build also checks by default it if is running inside of
** Microsoft IIS and if so forcing absolute redirects, which disable
** IIS internal redirection feature that is causing problems when
** cookies are getting set along with redirection.
*/
#if defined(ABSOLUTE_REDIRECT) || defined(CVSTRAC_WINDOWS)
#if !defined(ABSOLUTE_REDIRECT) && defined(CVSTRAC_WINDOWS)
  if(g.isIIS)
#endif
  {
...
/* Generates a Wiki textarea. */
void cgi_wikitext(
  const char *zName,      /* CGI param name */
  double rowpercent,      /* percentage of max configured height */
  const char *zContent    /* "default" text */
){
  int rows = atof(db_config("wiki_textarea_rows",WIKI_TEXTAREA_ROWS))
    * (rowpercent/100.0);
  if( rows < 5 ) rows = 5;

  if( !zName ) return;
  @ <textarea name="%h(zName)" \
  @ class="wikiedit" wrap="%h(db_config("wiki_textarea_wrap",WIKI_TEXTAREA_WRAP))" \
  @ cols="%h(db_config("wiki_textarea_cols",WIKI_TEXTAREA_COLS))" \
  @ rows="%d(rows)" \
  @ >%h(zContent?zContent:"")</textarea>
}

/*
** Generates a basic text entry area.
*/
void cgi_textarea(
  const char *zName,      /* CGI param name */
  const char *zClass,
  ...
  @ wrap="physical" cols="%d(cols)" rows="%d(rows)" \
  @ >%h(zContent?zContent:"")</textarea>
}
...
void cgi_http_server(int iPort){
  ...
  setsockopt(listener,SOL_SOCKET,SO_REUSEADDR,OS_VAL(&opt,(char *)&opt),sizeof(opt));
