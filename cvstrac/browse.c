  ...
  if( zPath && zPath[0] ){
    ...
    for(i=0; zDirUrl[i]; i++){
      if( zDirUrl[i]=='/' ){
        if( zDirName[0] ){
          zDirUrl[i] = 0;
          @ <li><a class="directory"
          @        href="%T(zDef)?d=%T(zDirUrl)">%h(zDirName)</a></li>
          zDirUrl[i] = '/';
        }
        zDirName = &zDirUrl[i];
        zDirName++;
    ...
    if( zDirName[0] ){
      if( isdir ){
        ...
        @ <li><a class="file" href="rlog?f=%T(zDirUrl)">%h(zDirName)</a></li>
      }
    }
...
static void revision_history(const char *zName, int showMilestones){
  ...
  for(i=0; az[i]; i+=9){
    ...
    if( az[i][0]==0 ){
      @ <tr class="milestone %s((i%2)?"odd":"")">
    }else if( i%2 ){
      @ <tr class="odd">
    }else{
      @ <tr>
    }
    @ <td class="date">%s(zDate)</td>
    if( az[i][0]==0 ){
      @ <td class="version">
...
static void column_header(
  ...
  */
  ...
  @     href="dirview?d=%T(zNameNS)&amp;o=%s(zField)%h(zDesc)">%h(zColumn)</a></th>
}
...
static void row_content(
  ...
  if( atoi(az[0])==1 ){
    @ <td colspan="3">
    @ <a href="dirview?d=%T(zName)%T(az[1])&amp;%h(zSortUrl)">
    common_icon("dir");
    @ %h(az[1])/</a></td>
...
void browse_dirview(void){
  ...
  */
  ...
  if( zName[0] ){
    @ <tr><td colspan="5">
    @ <a href="dirview?d=%T(zDir)&amp;%h(zSortUrl)">
    common_icon("backup");
    @ ..</a></td></tr>
  }
